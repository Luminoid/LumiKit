...
- **LMKDominantColorExtractor** — Histogram-backed dominant color extraction. Downsamples to a 40×40 grid, bins pixels into a 6×6×6 RGB histogram (216 buckets). Public APIs:
  ...
  - Migrated from Petfolio's `DominantColorExtractor`. Bug fix during migration: original code used `CGImageAlphaInfo.last` (unpremultiplied) for the lifted-subject path, which `CGBitmapContext` rejects on iOS — context creation returned nil and lifted extractions silently fell through to the full-image fallback. LumiKit always uses `.premultipliedLast` (the only supported RGBA format on iOS); fully-opaque pixels are unaffected, partially-transparent pixels are dropped by the alpha threshold
