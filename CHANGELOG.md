...
- **BREAKING: `LMKPhotoGridDataSource.photoGridImage(at:)` is now async** — `func photoGridImage(at index: Int) -> UIImage?` → `func photoGridImage(at index: Int) async -> UIImage?`, so providers can decode off the main actor instead of being forced into synchronous loads. Migration: add `async` to the implementation — a source holding a decoded image returns it unchanged; a disk- or network-backed source hops off the main actor (e.g. `Task.detached` + `UIImage.preparingForDisplay()`) and returns a ready-to-display image. Grid cells now show a neutral placeholder (`LMKColor.backgroundSecondary`) while loading, and apply results through a per-cell monotonic generation token, so a stale result never lands on a recycled cell. Sort, pinch-to-zoom columns, content-mode toggle, and LIVE-badge behavior are unchanged.
- **BREAKING: `LMKPhotoBrowserDataSource.photo(at:)` is now async** — `func photo(at index: Int) -> UIImage?` → `func photo(at index: Int) async -> UIImage?`, same executor contract as the grid. Migration: add `async` to the implementation (and `await` at any direct call sites). Pages show the browser background as a placeholder, install the loaded image through the standard sizing path (zoom-scale reset semantics unchanged), and are generation-token guarded against recycling; zoom, delete, and Live Photo upgrade flows work as before. Rotation re-fits use the cell's installed image instead of round-tripping the data source. `LMKSinglePhotoViewer` conforms by returning its in-memory image immediately.
- **SnapKit 6** — Dependency floor raised from 5.7.0 to 6.0.0. SnapKit 6.0.0's breaking changes are packaging-only (CocoaPods/Carthage removal, platform floors, Swift 6 toolchain); no source changes were needed at this package's iOS 18 floor.
...
- **LMKEmptyStateView action button** — `configure(message:icon:style:action:)` gains an optional `Action` (title, optional leading SF Symbol, `LMKButton.Style` — default filled primary — and a tap handler), rendered as an `LMKButton` centered below the message and hugging its content for the `.fullScreen` and `.card` styles; the horizontal `.inline` style ignores it. `setAction(_:)` adds, replaces, or removes the button after configure. The button joins the icon → message → button vertical constraint chain with its bottom closing the container, so the content-driven height grows with long messages and larger type sizes and the pieces can never overlap — hosts should stop anchoring their own buttons to the view's bottom edge. The button fades in after the message, honoring Reduce Motion. Accessibility: with an action present the view stops being a single static-text element (which would swallow the button) and becomes a container exposing the message and button separately; the single-element behavior returns when the action is nil.
- **LMKScrollStackViewController navigation bar + keyboard hooks** — Overridable `navigationBar: LMKNavigationBar?` (default nil) installs a custom bar via `pinToTop(of:)` with the scroll view topping out at the bar's bottom instead of the view's top edge, and `installsKeyboardAdjustment` (default `true`) installs `lmk_enableKeyboardAdjustment()` on the scroll view. Existing subclasses keep pixel-identical layout with both defaults; keyboard avoidance is now active out of the box.
- **LMKFormScaffold** — Static builders for form screens that are not `LMKScrollStackViewController` subclasses: `makeScrollView(keyboardDismissMode:)` (default `.onDrag`, keyboard-overlap avoidance pre-installed), `makeContentStack(spacing:)` (vertical, token spacing), and `install(scrollView:stack:in:below:contentInsets:)` pinning the pair into a host view below an optional top anchor (e.g. a custom navigation bar) with token insets.
- **LMKBottomSheetController.avoidsKeyboard** — Default `true`: the sheet lifts by the keyboard's actual overlap with its view (converted end frame — floating keyboards and short or side-by-side windows lift only as much as they're covered) via `containerBottomConstraint`, animating with the keyboard's own curve and duration, and restores on hide. Composes with drag-to-dismiss (pan-start resigns the first responder, so pan offsets stay absolute against the resting position) and dismissal stops keyboard tracking so a mid-animation hide can't fight the slide-out. While enabled, the controller owns the keyboard portion of the bottom offset — subclasses with hand-rolled keyboard observers should delete them, or return `false` for fully manual control. `LMKKeyboardObserver.KeyboardInfo` gains a `frameEnd` rect backing the overlap computation.
- **LMKLayout.hairline** — `hairline(forScale:)` returns one physical pixel (0.5 on 2x, ~0.33 on 3x; scales at or below 1 clamp to a full point), and `hairline` resolves the key window's screen scale with a 2x fallback. Deliberately not a theme axis: display physics, not branding. `LMKNavigationBar`'s separator now uses the token.
- **Icon list row pointer hover** — `lmk_configureIconListRow` gains `pointerEnabled: Bool = true`, installing a `UIPointerInteraction` on the cell once (associated-object guard against repeated reconfiguration) whose hover style routes through `LMKPointerStyle`, so a cell recycled mid-hover can't trip `UITargetedPreview`'s window assertion. Inert on platforms without pointer support.
...
- **LMKEmptyStateView bottom-anchored CTA overlap** — The action-button layout flaw this release closes: content centered in a flexible-height frame meant a host-anchored button under the view could overlap a spilled multi-line message at larger Dynamic Type sizes. The built-in `Action` participates in the constraint chain, so the height is content-driven at every content size.
- **LMKEmptyStateView zero-height collapse** — The centered content container had no vertical tie to the view's own edges, so with no host-imposed height the view collapsed to zero height and its content spilled over neighboring views (most visibly as arranged overlap in stack layouts). Containment guards (priority 999, so a deliberately short host still gets silent overflow instead of unsatisfiable-constraint breakage) plus below-required edge hugging now give the view its content height when the host imposes none; a host-imposed height still wins and centers the content.
- **LMKEmptyStateView live Dynamic Type** — The message label now sets `adjustsFontForContentSizeCategory`, so a mid-session content-size change reflows the message immediately instead of waiting for the next `configure()` call.

### Infrastructure

- **lottie-spm resolve note** — Package.swift documents that SwiftPM's full-graph resolution fetches the Lottie package's metadata for all consumers, while the binary artifact downloads only when `LumiKitLottie` is linked. Dependency version unchanged; no restructuring (package traits are not selectable from app projects consuming a local package).
- 1008 → 1070 tests (147 suites); 120 → 122 source files (`LMKPointerStyle.swift`, `LMKFormScaffold.swift`).
- Example app: the photo grid page now demonstrates the async data source pattern end to end (JPEG bytes → off-main decode → ready-to-display image), and the photo browser page shows the immediate-return shape for in-memory images.
